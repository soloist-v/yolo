...
def get_pair_sample_from_dir(img_dir, label_dirs, formats):
    img_dir = Path(img_dir).as_posix()
    label_dirs = [Path(label_dir).as_posix() for label_dir in label_dirs]
    ...
    for name, img_path in walk_dir(img_dir):
        ...
        if ext not in img_formats:
            continue
        filenames = []
        for label_dir, format_str in zip(label_dirs, formats):
            filenames.append(format_str % (label_dir, prefix[img_dir_len:]))
        yield name, img_path, *filenames
...
def yolo2xml(img_file, txt_file, xml_file, names):
    ...
    if img is None:
        return
    size = img.shape[:2][::-1]
    content = open(txt_file, 'rb').read().decode("utf8")
    xml = VocXML.create()
...
def yolo2xml_dir(img_dir, txt_dir, save_dir, names):
    os.makedirs(save_dir, exist_ok=True)
    for name, img_path, txt_path, xml_path in get_pair_sample_from_dir(img_dir, [txt_dir, save_dir],
                                                                       ["%s%s.txt", "%s%s.xml"]):
        yolo2xml(img_path, txt_path, xml_path, names)
...
if __name__ == '__main__':
    ...
    if is_clear and Path(save_dir).exists():
        shutil.rmtree(save_dir)
    yolo2xml_dir(img_dir, txt_dir, save_dir, names)
